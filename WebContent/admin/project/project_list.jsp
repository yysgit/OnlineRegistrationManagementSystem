	...
	<div class="page-content">
		<form action="<%=path%>/project_list.action" class="form-inline" method="post">
			<div class="panel panel-default">
				...
				<pg:pager url="project_list.action" items="${itemSize}" maxPageItems="${pageItem}" maxIndexPages="${pageItem}" isOffset="${true}" export="offset,currentPageNumber=pageNumber" scope="request">
							...
							<tbody>
								<c:forEach items="${list}" var="info">
									<tr>
										...
										<td>
											<a href="<%=path%>/project_toUpdate.action?id=${info.id}"
...
$(document).ready(function(){
   $("#addBtn").click(function(){
        window.location.href = '<%=path%>/project_toAdd.action';
   });
});
function delInfo(id){
	 if(confirm("是否确认删除？")){
	     window.location.href = '<%=path%>/project_delete.action?id=' + id;
	 }
	}
